  ...
  function renderizarDados() {
    return props.clientes?.map((cliente, i) => {
      return (
        ...
        >
          <td className="text-lelt p-4">{cliente.id}</td>
          <td className="text-lelt p-4">{cliente.nome}</td>
          <td className="text-lelt p-4">{cliente.idade}</td>
      ...
      <td className="flex justify-center">
        ...
        {props.clienteExcluido ? (
          <button onClick={() => props.clienteExcluido?.(cliente)}
            className={`
          flex justify-center items-center text-red-500 rounded-full hover:bg-purple-50 p-2 m-1
          `}
          >
            {IconeLixo}
          </button>
